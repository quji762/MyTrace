...
/// deviceFlow: .standard — the RFC 8628 specification's own shape (unlike OpenAI's).
...
/// looks right and is REFUSED by the endpoint — do not add it.
/// </summary>
public sealed class GrokDeviceLogin
{
    ...
    public async Task<DevicePrompt> StartAsync(CancellationToken cancellationToken = default)
    {
        ...
        // decision (GitHub deliberately sends none — that is the difference).
    ...
    /// <summary>
    /// RFC 8628 polling: waiting and refusal BOTH arrive as a 400 — the body's
...
/// party. 1) open the login page with a challenge+uuid, 2) poll — **404 means "not
