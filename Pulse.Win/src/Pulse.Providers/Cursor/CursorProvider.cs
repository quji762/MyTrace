...
/// - `autoPercentUsed` 0.0267 means 0.0267%, NOT 2.67% — a fraction would be a
///   hundredfold overstatement (settled by arithmetic upstream).
/// - Pools' 30 days are a sort key (billing cycles run 28–31 days):
...
public sealed class CursorProvider : HttpUsageProviderBase
{
    ...
    protected override IReadOnlyDictionary<string, string> ExtraHeaders() =>
        new Dictionary<string, string>
        {
            ...
            // reaches here — TryAddWithoutValidation would drop the request.)
...
public static class CursorMapping
{
    ...
    public static UsageWindow? Pool(double? percent, string id, string scope, DateTimeOffset? resets)
    {
        ...
        return new UsageWindow(
            ...
            // A billing cycle runs 28–31 days; thirty only orders the rows.
    ...
    /// <summary>"pro_plus" → "Pro+". Unfamiliar tiers tidied and passed through.</summary>
